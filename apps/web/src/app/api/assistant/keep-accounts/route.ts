...
import { assertContaIdsInGestao } from "@/lib/server/permissions";
import { deactivateContasExcept } from "@/lib/server/repository";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const denied = gestaoAccessDeniedResponse(error, {
      userId,
      gestaoId,
      route: "/api/assistant/keep-accounts",
    });
    if (denied) {
      return denied;
    }
  ...
  try {
    await assertContaIdsInGestao(parsed.data.manterContaIds, gestaoId);
  } catch (error) {
    const denied = gestaoAccessDeniedResponse(error, {
      userId,
      gestaoId,
      route: "/api/assistant/keep-accounts",
      entityCount: parsed.data.manterContaIds.length,
    });
    if (denied) {
      return denied;
    }
